...
#ifndef	JOS_KERN_NVMEREG_H
#define	JOS_KERN_NVMEREG_H
...
#define  NVME_CAP_CSS_NVM	BIT(0)
#define  NVME_CAP_NSSRS(_r)	ISSET((_r), BIT(36))
#define  NVME_CAP_DSTRD(_r)	BIT(2 + (((_r) >> 32) & 0xf)) /* bytes */
#define  NVME_CAP_TO(_r)	(500 * (((_r) >> 24) & 0xff)) /* ms */
#define  NVME_CAP_AMS(_r)	(((_r) >> 17) & 0x3)
#define  NVME_CAP_AMS_WRR	BIT(0)
#define  NVME_CAP_AMS_VENDOR	BIT(1)
#define  NVME_CAP_CQR(_r)	ISSET((_r), BIT(16))
...
struct nvme_cqe {
	...
	uint16_t	flags;
#define NVME_CQE_DNR		BIT(15)
#define NVME_CQE_M		BIT(14)
#define NVME_CQE_SCT_MASK	BITS(8, 10)
...
#define NVME_CQE_SC_MASK	BITS(1, 7)
...
struct nvm_identify_psd {
	...
	uint8_t		_reserved1;
	uint8_t		flags;
#define	NVME_PSD_NOPS		BIT(1)
#define	NVME_PSD_MPS		BIT(0)
...
#define	NVME_PSD_RRT_MASK	BITS(0, 4)
	uint8_t		rrl;		/* Relative Read Latency */
#define	NVME_PSD_RRL_MASK	BITS(0, 4)
	uint8_t		rwt;		/* Relative Write Throughput */
#define	NVME_PSD_RWT_MASK	BITS(0, 4)
	uint8_t		rwl;		/* Relative Write Latency */
#define	NVME_PSD_RWL_MASK	BITS(0, 4)

	uint16_t	idlp;		/* Idle Power */
	uint8_t		ips;		/* Idle Power Scale */
#define	NVME_PSD_IPS_MASK	BITS(0, 1)
	uint8_t		_reserved2;
	uint16_t	actp;		/* Active Power */
	uint16_t	ap;		/* Active Power Workload/Scale */
#define	NVME_PSD_APW_MASK	BITS(0, 2)
#define	NVME_PSD_APS_MASK	BITS(6, 7)
...
#define	NVME_ID_CTRLR_OACS_NS		BIT(3)
#define	NVME_ID_CTRLR_OACS_FW		BIT(2)
#define	NVME_ID_CTRLR_OACS_FORMAT	BIT(1)
#define	NVME_ID_CTRLR_OACS_SECURITY	BIT(0)
	uint8_t		acl;		/* Abort Command Limit */
	uint8_t		aerl;		/* Asynchronous Event Request Limit */

	uint8_t		frmw;		/* Firmware Updates */
#define	NVME_ID_CTRLR_FRMW_NOREQ_RESET	BIT(4)
#define	NVME_ID_CTRLR_FRMW_NSLOT	BITS(1, 3)
#define	NVME_ID_CTRLR_FRMW_SLOT1_RO	BIT(0)
	uint8_t		lpa;		/* Log Page Attributes */
#define	NVME_ID_CTRLR_LPA_CMD_EFFECT	BIT(1)
#define	NVME_ID_CTRLR_LPA_NS_SMART	BIT(0)
...
#define	NVME_ID_CTRLR_SQES_MAX		BITS(4, 7)
#define	NVME_ID_CTRLR_SQES_MIN		BITS(0, 3)
	uint8_t		cqes;		/* Completion Queue Entry Size */
#define	NVME_ID_CTRLR_CQES_MAX		BITS(4, 7)
#define	NVME_ID_CTRLR_CQES_MIN		BITS(0, 3)
	uint8_t		_reserved3[2];

	uint32_t	nn;		/* Number of Namespaces */

	uint16_t	oncs;		/* Optional NVM Command Support */
#define	NVME_ID_CTRLR_ONCS_RESERVATION	BIT(5)
#define	NVME_ID_CTRLR_ONCS_SET_FEATURES	BIT(4)
#define	NVME_ID_CTRLR_ONCS_WRITE_ZERO	BIT(3)
#define	NVME_ID_CTRLR_ONCS_DSM		BIT(2)
#define	NVME_ID_CTRLR_ONCS_WRITE_UNC	BIT(1)
#define	NVME_ID_CTRLR_ONCS_COMPARE	BIT(0)
	uint16_t	fuses;		/* Fused Operation Support */

	uint8_t		fna;		/* Format NVM Attributes */
	uint8_t		vwc;		/* Volatile Write Cache */
#define	NVME_ID_CTRLR_VWC_PRESENT	BIT(0)
...
#define	NVME_ID_NS_NSFEAT_LOGICAL_BLK_ERR	BIT(2)
#define	NVME_ID_NS_NSFEAT_NS			BIT(1)
#define	NVME_ID_NS_NSFEAT_THIN_PROV		BIT(0)
