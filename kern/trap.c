...
trap_init(void)
{
  ...
  SETGATE(idt[T_SYSCALL], 0, GD_KT, &syscallh, 3);
  SETGATE(idt[T_DEFAULT], 0, GD_KT, &simderr, 3);
...
trap_dispatch(struct Trapframe *tf)
{
  ...
  if (tf->tf_trapno == T_SYSCALL) {
    ...
    int ret = syscall(syscall_num, arg1, arg2, arg3, arg4, arg5);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
  if (tf->tf_cs == GD_KT) panic("page fault in kernel mode");
  ...
  if (tf->tf_esp >= UXSTACKTOP-PGSIZE && tf->tf_esp <= UXSTACKTOP-1) {
    utf = (struct UTrapframe *) (tf->tf_esp - sizeof(struct UTrapframe) - sizeof(uintptr_t));
    user_mem_assert(curenv, (void *) utf, sizeof(struct UTrapframe) + sizeof(uintptr_t), PTE_P | PTE_U | PTE_W);
  } else {
